...
export class ChatFrontendStack extends Stack {
  constructor(scope: Construct, id: string, props: ChatFrontendStackProps) {
    ...
    // Solo agrega domainNames si hay un dominio configurado
    if (config.domainName) {
      distributionProps.domainNames = [config.domainName];
    }

    const distribution = new Distribution(this, 'Distribution', distributionProps);
